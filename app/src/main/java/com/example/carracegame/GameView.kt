        ...
        if (canvas != null) {
            ...
            for (i in otherCars.indices) {
                try {
                    ...
                    val d2 = resources.getDrawable(R.drawable.cars_blue, null)
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        when(event?.action) {
            MotionEvent.ACTION_DOWN -> {
                val x1 = event.x
                val laneWidth = viewWidth / 3 // Divide the screen width into 3 lanes
                val lane = (x1 / laneWidth).toInt() // Determine which lane was touched

                // Update the car position based on the lane touched
                myCarPosition = lane.coerceIn(0, 2)

                invalidate() // Redraw the view to reflect the new car position
            }
